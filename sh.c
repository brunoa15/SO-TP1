...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     * 
     * Resposta: o if testa se a chamada de sistema chdir retorna um erro,
     * ou seja, se o diretório não é válido ou não encontrado. 
     * 
     *  */
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "Diretório não válido ou não encontrado.\n");
      continue;
    }
    /* MARK END task1 */
